...
regex_match(cmdline,"curl\s+.*-d\s+\S+",1) IS NOT NULL OR
regex_match(cmdline,"curl\s+.*--data-raw\s+\S+",1) IS NOT NULL OR
regex_match(cmdline,"curl\s+.*https?:\/\/\S+:\S+@",0) IS NOT NULL OR
regex_match(cmdline,"curl\s+.*Authorization:\s+Bearer\s+\S+",0) IS NOT NULL OR
